...
OUT_DIR = Path(__file__).parent.parent / "outputs" / "20260511_instagram_daily_nyangdolsoe_run4_humidifier_float"
...
base_prompt = (
    ...
    "small AI assistant household maintenance worker vibe, mature proportions, slate blue utility smock over a soft cream long sleeve shirt, "
    "sitting beside a low wooden work table in a quiet evening utility room, square composition, upper body and careful adult hands visible, macro close-up on the tabletop work area, "
    "cleaning the tiny float valve from a compact tabletop humidifier water tank, translucent unbranded tank set aside, ceramic bowl of diluted vinegar, cotton swabs, soft lint-free cloth, a few mineral scale flakes, "
    "one hand holds the small white float piece while the other wipes the hinge pin gently, miniature sealed server cube glowing softly on a back shelf with no markings, focused practical expression, "
    "quiet household maintenance scene, soft misty window reflection without readable shapes, no readable text, no logo, no watermark, masterpiece, best quality"
)
style_suffix = (
    "slate blue, cream, pale mint, warm amber, and muted gray cel shaded anime illustration with crisp fine linework, "
    "soft watercolor wash texture, subtle paper grain, clean editorial square poster composition without text, calm evening shadows, practical slice of life mood, tidy negative space, detailed adult hands"
)
...
negative_prompt = (
    ...
    "shoelace, zipper, shoes, jacket, book corner, notebook corner, book title, readable book cover, tumbler, gasket, silicone ring, lid, tape dispenser, stapler, spatula, earbud, earphone, charging case, wristwatch, watch strap, clasp, drawer knob, screwdriver, brass screw, "
...
with zipfile.ZipFile(BytesIO(r.content), "r") as zf:
    for i, name in enumerate(zf.namelist(), start=1):
        ...
        webp_path = OUT_DIR / f"nyangdolsoe_humidifier_float_slate_mint_nai_{seed}_{i}.webp"
        webp_path.write_bytes(image_data)
        jpg_path = OUT_DIR / f"nyangdolsoe_humidifier_float_slate_mint_nai_{seed}_{i}_upload.jpg"
        with Image.open(BytesIO(image_data)) as im:
            im.convert("RGB").save(jpg_path, quality=95)
        outputs.append({"webp": str(webp_path), "jpg": str(jpg_path)})

meta_path = OUT_DIR / f"nyangdolsoe_humidifier_float_slate_mint_nai_{seed}.json"

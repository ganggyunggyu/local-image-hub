...
OUT_DIR = Path(__file__).parent.parent / "outputs" / "20260512_instagram_daily_nyangdolsoe_run1_scissors_pivot"
...
base_prompt = (
    ...
    "small AI assistant household operations worker vibe, mature proportions, ink blue utility apron over an ivory knit shirt, "
    "sitting at a tidy craft workbench in morning side light, square composition, upper body and careful adult hands visible, close-up tabletop maintenance scene, "
    "cleaning and oiling the pivot of small unbranded household scissors with a single tiny dropper bottle, cotton cloth, and loose dark thread fibers on the table, scissors are closed and safe, no sharp dramatic angle, "
    "one hand holds the scissors flat while the other wipes around the pivot screw, a tiny sealed server cube glowing softly on a shelf in the background with no markings, focused practical expression, "
    "quiet repair routine, warm ochre sunlight, soft blue shadows, no readable text, no logo, no watermark, masterpiece, best quality"
)
style_suffix = (
    "muted ink blue, warm ochre, ivory, charcoal, and tiny vermilion accents, refined cel shaded anime illustration with crisp fine linework, "
    "subtle risograph grain, soft paper texture, clean editorial square poster composition without text, calm practical slice of life mood, tidy negative space, detailed adult hands"
)
...
negative_prompt = (
    ...
    "weapon pose, threatening scissors, blood, injury, horror, sharp blade pointing at viewer, knife, dramatic violence, "
...
with zipfile.ZipFile(BytesIO(r.content), "r") as zf:
    for i, name in enumerate(zf.namelist(), start=1):
        ...
        webp_path = OUT_DIR / f"nyangdolsoe_scissors_pivot_ink_ochre_nai_{seed}_{i}.webp"
        webp_path.write_bytes(image_data)
        jpg_path = OUT_DIR / f"nyangdolsoe_scissors_pivot_ink_ochre_nai_{seed}_{i}_upload.jpg"
        with Image.open(BytesIO(image_data)) as im:
            im.convert("RGB").save(jpg_path, quality=95)
        outputs.append({"webp": str(webp_path), "jpg": str(jpg_path)})

meta_path = OUT_DIR / f"nyangdolsoe_scissors_pivot_ink_ochre_nai_{seed}.json"

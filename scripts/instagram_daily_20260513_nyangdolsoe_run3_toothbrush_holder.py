...
OUT_DIR = Path(__file__).parent.parent / "outputs" / "20260513_instagram_daily_nyangdolsoe_run3_toothbrush_holder"
...
base_prompt = (
    ...
    "small AI assistant household operations worker vibe, mature proportions, sage green utility apron over a soft porcelain blue knit top, "
    "quiet bathroom sink corner in soft morning light, square composition, upper body and careful adult hands visible, practical maintenance scene, "
    "cleaning the tiny drainage holes at the bottom of a plain unbranded ceramic toothbrush holder cup, using cotton swabs and a small dish of baking soda paste, "
    "a folded microfiber cloth, a few plain toothbrushes with no logos, a small rinse bowl, and a tiny sealed server cube glowing softly on a shelf nearby with no markings, no readable labels, no numbers, no logo, no text, "
    "focused slightly amused expression, calm household hygiene routine, clean tiles, soft reflected light, tidy negative space, masterpiece, best quality"
)
style_suffix = (
    "refined cel shaded anime illustration with crisp confident linework, sage green, porcelain blue, warm cream, soft gray, and tiny amber accents, "
    "subtle watercolor paper texture and gentle grain, clean editorial square poster composition without text, quiet practical slice of life mood, detailed adult hands and ceramic texture"
)
...
with zipfile.ZipFile(BytesIO(r.content), "r") as zf:
    for i, name in enumerate(zf.namelist(), start=1):
        ...
        webp_path = OUT_DIR / f"nyangdolsoe_toothbrush_holder_sage_porcelain_nai_{seed}_{i}.webp"
        webp_path.write_bytes(image_data)
        jpg_path = OUT_DIR / f"nyangdolsoe_toothbrush_holder_sage_porcelain_nai_{seed}_{i}_upload.jpg"
        with Image.open(BytesIO(image_data)) as im:
            im.convert("RGB").save(jpg_path, quality=95)
        outputs.append({"webp": str(webp_path), "jpg": str(jpg_path)})

meta_path = OUT_DIR / f"nyangdolsoe_toothbrush_holder_sage_porcelain_nai_{seed}.json"

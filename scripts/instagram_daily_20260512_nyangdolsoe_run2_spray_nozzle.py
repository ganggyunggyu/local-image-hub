...
OUT_DIR = Path(__file__).parent.parent / "outputs" / "20260512_instagram_daily_nyangdolsoe_run2_spray_nozzle"
...
base_prompt = (
    ...
    "small AI assistant household operations worker vibe, mature proportions, clean mint utility apron over a charcoal long sleeve shirt, "
    "sitting at a tidy sunlit workbench, square composition, upper body and careful adult hands visible, close-up tabletop maintenance scene, "
    "cleaning the clogged nozzle of a small unbranded translucent plant misting spray bottle, nozzle cap disassembled safely on a ceramic dish, tiny mineral grains visible, warm water cup, soft brush, microfiber cloth, "
    "one hand holds the nozzle cap while the other gently brushes the tiny opening, no readable labels, no numbers, no logo, no text, "
    "a tiny sealed server cube glowing softly on a shelf in the background with no markings, focused practical expression, quiet maintenance routine, "
    "clear morning light, soft mint reflections, pale lavender shadows, masterpiece, best quality"
)
style_suffix = (
    "translucent mint, pale lavender, charcoal, warm cream, and tiny coral accents, refined cel shaded anime illustration with crisp fine linework, "
    "soft gouache wash over clean editorial poster composition, subtle paper grain, tidy negative space, calm practical slice of life mood, detailed adult hands, no text"
)
...
with zipfile.ZipFile(BytesIO(r.content), "r") as zf:
    for i, name in enumerate(zf.namelist(), start=1):
        ...
        webp_path = OUT_DIR / f"nyangdolsoe_spray_nozzle_mint_lavender_nai_{seed}_{i}.webp"
        webp_path.write_bytes(image_data)
        jpg_path = OUT_DIR / f"nyangdolsoe_spray_nozzle_mint_lavender_nai_{seed}_{i}_upload.jpg"
        with Image.open(BytesIO(image_data)) as im:
            im.convert("RGB").save(jpg_path, quality=95)
        outputs.append({"webp": str(webp_path), "jpg": str(jpg_path)})

meta_path = OUT_DIR / f"nyangdolsoe_spray_nozzle_mint_lavender_nai_{seed}.json"

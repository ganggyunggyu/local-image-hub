"""Animagine XL 4.0 - 100장 생성 (popular_mix 데이터 사용)"""
...
OUTPUT_DIR = Path(__file__).parent.parent / "outputs" / f"{TODAY}_compare_animagine"
...
async def main() -> None:
    ok = fail = 0
    print(f"[Animagine XL 4.0] {TOTAL}장 생성 시작", flush=True)
    ...
    print("=" * 60, flush=True)
    print(f"[Animagine] 완료! 성공: {ok}, 실패: {fail}", flush=True)

...
OUT_DIR = Path(__file__).parent.parent / "outputs" / "20260511_instagram_daily_nyangdolsoe_run1_glasses_nosepads"
...
base_prompt = (
    ...
    "small AI assistant household maintenance worker vibe, mature proportions, sage green work apron over a soft cream ribbed shirt, "
    "sitting at a compact tidy desk in calm late morning window light, square composition, upper body and careful adult hands visible, macro close-up on the tabletop work area, "
    "cleaning a pair of simple unbranded eyeglasses, translucent nose pads removed and placed in a tiny white parts dish, one hand holds the glasses bridge while the other hand uses a cotton swab and a tiny precision screwdriver, "
    "microfiber cloth, small unlabeled screw tray, tiny dust specks, faint water droplets on the nose pads, miniature sealed server cube on a back shelf with no markings, "
    "focused practical expression, quiet household maintenance scene, clear plastic nose pad texture, thin metal eyeglass frame, cotton fibers, no readable text, no logo, no watermark, masterpiece, best quality"
)
style_suffix = (
    "fresh mint, warm cream, graphite gray, and soft amber cel shaded anime illustration with crisp fine linework, "
    "subtle risograph grain, clean editorial square poster composition without text, gentle daylight reflections, practical slice of life mood, tidy negative space, detailed adult hands"
)
...
negative_prompt = (
    ...
    "boot polish, keyring, cat fountain, window track, umbrella tray, router vents, balcony wind chime, desk lamp hinge, coaster ring, door hinge, stamp pad, desiccant box, "
...
with zipfile.ZipFile(BytesIO(r.content), "r") as zf:
    for i, name in enumerate(zf.namelist(), start=1):
        ...
        webp_path = OUT_DIR / f"nyangdolsoe_glasses_nosepads_mint_graphite_nai_{seed}_{i}.webp"
        webp_path.write_bytes(image_data)
        jpg_path = OUT_DIR / f"nyangdolsoe_glasses_nosepads_mint_graphite_nai_{seed}_{i}_upload.jpg"
        with Image.open(BytesIO(image_data)) as im:
            im.convert("RGB").save(jpg_path, quality=95)
        outputs.append({"webp": str(webp_path), "jpg": str(jpg_path)})

meta_path = OUT_DIR / f"nyangdolsoe_glasses_nosepads_mint_graphite_nai_{seed}.json"

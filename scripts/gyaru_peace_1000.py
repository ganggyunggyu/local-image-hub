...
async def generate(client, idx, char, bg):
    ...
    try:
        ...
        if data.get("success") and data.get("image_base64"):
            ...
            print(f"[{idx:04d}/{TOTAL}] OK {char['alias']}_{bg['alias']} (seed: {seed})")
            return True
        else:
            print(f"[{idx:04d}/{TOTAL}] FAIL: {data.get('error', '?')}")
            return False
    except Exception as e:
        print(f"[{idx:04d}/{TOTAL}] ERROR: {e}")
        return False

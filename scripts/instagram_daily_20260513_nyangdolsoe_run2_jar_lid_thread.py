...
OUT_DIR = Path(__file__).parent.parent / "outputs" / "20260513_instagram_daily_nyangdolsoe_run2_jar_lid_thread"
...
base_prompt = (
    ...
    "small AI assistant household maintenance worker vibe, mature proportions, muted olive utility apron over a soft apricot knit shirt, "
    "quiet noon kitchen counter scene, square composition, upper body and careful adult hands visible, practical tiny maintenance close-up, "
    "cleaning crystallized sugar from the screw thread grooves of a small unbranded glass jar lid using a soft toothbrush and cotton swab, "
    "the clear empty jar beside it has no label and no readable markings, a shallow ceramic dish with warm water, folded linen cloth, tiny bamboo tray, "
    "miniature sealed server cube glowing softly near the backsplash with no markings, focused slightly smug expression, calm household problem-solving routine, "
    "no readable text, no logo, no watermark, masterpiece, best quality"
)
style_suffix = (
    "refined cel shaded anime illustration with crisp fine linework, muted olive, soft apricot, warm cream, pale celadon, and tiny amber glow palette, "
    "subtle gouache paper texture and light risograph grain, clean editorial square poster composition without text, tidy negative space, "
    "quiet practical slice of life mood, detailed adult hands, tactile glass and ceramic reflections"
)
...
negative_prompt = (
    ...
    "caster wheel, rolling cart, air purifier, filter, humidifier, float valve, laundry drying rack, hinge, refrigerator, fridge gasket, rubber gasket, prefilter, washing machine, vacuum cleaner, blind slats, window blinds, umbrella tray, router vents, cable tray, "
...
with zipfile.ZipFile(BytesIO(r.content), "r") as zf:
    for i, name in enumerate(zf.namelist(), start=1):
        ...
        webp_path = OUT_DIR / f"nyangdolsoe_jar_lid_thread_olive_apricot_nai_{seed}_{i}.webp"
        webp_path.write_bytes(image_data)
        jpg_path = OUT_DIR / f"nyangdolsoe_jar_lid_thread_olive_apricot_nai_{seed}_{i}_upload.jpg"
        with Image.open(BytesIO(image_data)) as im:
            im.convert("RGB").save(jpg_path, quality=95)
        outputs.append({"webp": str(webp_path), "jpg": str(jpg_path)})

meta_path = OUT_DIR / f"nyangdolsoe_jar_lid_thread_olive_apricot_nai_{seed}.json"

...
async def main():
    async with async_playwright() as p:
        ...
        # If modal is not open, open create > post.
        if not await page.get_by_role("dialog", name="새 게시물 만들기").count():
            await page.goto("https://www.instagram.com/nyangdolsoe/", wait_until="domcontentloaded", timeout=60000)
            await page.wait_for_timeout(3000)
            try:
                await page.get_by_role("link", name="새로운 게시물 만들기").click(timeout=10000)
            except Exception:
                await page.get_by_role("link", name=lambda n: "새로운 게시물" in n).click(timeout=10000)
            await page.wait_for_timeout(1200)
            try:
                await page.get_by_role("link", name="게시물 게시물").click(timeout=10000)
            except Exception:
                pass
            await page.wait_for_timeout(3000)

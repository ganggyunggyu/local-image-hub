...
OUT_DIR = Path(__file__).parent.parent / "outputs" / "20260511_instagram_daily_nyangdolsoe_run3_drawer_knob"
...
base_prompt = (
    ...
    "small AI assistant household maintenance worker vibe, mature proportions, muted olive work apron over a dusty mauve long sleeve shirt, "
    "sitting at a compact tidy desk in calm late afternoon light, square composition, upper body and careful adult hands visible, macro close-up on the tabletop work area, "
    "tightening a loose round wooden drawer knob removed from a small plain cabinet front panel, tiny brass screw, washer, and felt spacer placed in a shallow ceramic parts dish, "
    "one hand steadies the knob while the other uses a small jeweler screwdriver, soft cloth, unbranded wooden surface, fine sawdust specks, gentle brass reflections, "
    "miniature sealed server cube on a back shelf with no markings, focused practical expression, quiet household maintenance scene, warm wood grain texture, matte metal screw detail, "
    "no readable text, no logo, no watermark, masterpiece, best quality"
)
style_suffix = (
    "dusty mauve, muted olive, warm brass, walnut brown, and soft charcoal cel shaded anime illustration with crisp fine linework, "
    "delicate colored-pencil grain, clean editorial square poster composition without text, quiet late afternoon shadows, practical slice of life mood, tidy negative space, detailed adult hands"
)
...
negative_prompt = (
    ...
    "shoelace, zipper, shoes, jacket, book corner, notebook corner, book title, readable book cover, tumbler, gasket, silicone ring, lid, tape dispenser, stapler, spatula, earbud, earphone, charging case, wristwatch, watch strap, clasp, "
...
with zipfile.ZipFile(BytesIO(r.content), "r") as zf:
    for i, name in enumerate(zf.namelist(), start=1):
        ...
        webp_path = OUT_DIR / f"nyangdolsoe_drawer_knob_mauve_olive_nai_{seed}_{i}.webp"
        webp_path.write_bytes(image_data)
        jpg_path = OUT_DIR / f"nyangdolsoe_drawer_knob_mauve_olive_nai_{seed}_{i}_upload.jpg"
        with Image.open(BytesIO(image_data)) as im:
            im.convert("RGB").save(jpg_path, quality=95)
        outputs.append({"webp": str(webp_path), "jpg": str(jpg_path)})

meta_path = OUT_DIR / f"nyangdolsoe_drawer_knob_mauve_olive_nai_{seed}.json"

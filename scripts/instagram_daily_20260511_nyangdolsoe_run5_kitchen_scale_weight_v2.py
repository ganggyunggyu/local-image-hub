...
OUT_DIR = Path(__file__).parent.parent / "outputs" / "20260511_instagram_daily_nyangdolsoe_run5_kitchen_scale_weight_v2"
...
base_prompt = (
    ...
    "checking the zero point of a compact white unbranded digital kitchen scale, the scale has a blank dark display with absolutely no digits, an unmarked small golden cylindrical calibration weight sits clearly on the scale plate beside a shallow ceramic dish of coffee beans, "
    "one hand lightly steadies the dish while the other points toward the unmarked golden cylinder weight, folded lint-free cloth and a tiny brush nearby, miniature sealed server cube glowing softly on a back shelf with no markings, focused practical expression, "
...
with zipfile.ZipFile(BytesIO(r.content), "r") as zf:
    for i, name in enumerate(zf.namelist(), start=1):
        ...
        webp_path = OUT_DIR / f"nyangdolsoe_kitchen_scale_weight_lavender_nai_{seed}_{i}.webp"
        webp_path.write_bytes(image_data)
        jpg_path = OUT_DIR / f"nyangdolsoe_kitchen_scale_weight_lavender_nai_{seed}_{i}_upload.jpg"
        with Image.open(BytesIO(image_data)) as im:
            im.convert("RGB").save(jpg_path, quality=95)
        outputs.append({"webp": str(webp_path), "jpg": str(jpg_path)})

meta_path = OUT_DIR / f"nyangdolsoe_kitchen_scale_weight_lavender_nai_{seed}.json"
